...
if __name__ == '__main__':
    ...
    model = torch.load('predictor-backbone-cycle-7.pth')
    labeled_setn = np.load('labelled-head6.npy')
    labeled_setprev_ind = np.load('labelled-head5.npy')
    ...
    with torch.no_grad():
        ...
        plt.title('WEBNLG, VAAL - Cycle 7')
        plt.tight_layout()
        plt.legend()
        plt.savefig('tsne_plot_VAAL_cycle7.png')
        plt.show()

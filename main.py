...
if __name__ == '__main__':
    ...
    adden = 100 # roughly no_train/cycles
    ...
    if args.do_train:
        ...
        for cycle in range(7):
            ...
            new_list = list(torch.tensor(subset)[arg][:20].numpy())
            labeled_set += list(torch.tensor(subset)[arg][-20:].numpy())
            listd = list(torch.tensor(subset)[arg][:-20].numpy()) 
            unlabeled_set = listd + unlabeled_set[20:]
